...
function Trust({ type, name }) { if (!type || type === 'CACHED' || type === 'DEMO') return null; return <span className={'source ' + trustTone(type)}>{type}{name ? ` • ${name}` : ''}</span>; }
...
function IndianFlag({ className = '' }) {
    const spokes = Array.from({ length: 24 }, (_, i) => <line key={i} x1="12" y1="4.6" x2="12" y2="19.4" transform={`rotate(${i * 15} 12 12)`}/>);
    return <span className={`india-flag ${className}`} role="img" aria-label="Indian flag"><span className="flag-band saffron"/><span className="flag-band white"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="7.4"/><circle cx="12" cy="12" r="1.5" fill="#1f4f91" stroke="none"/>{spokes}</svg></span><span className="flag-band green"/></span>;
}
const citizenSampleReports = [
    { _id: 'citizen-report-1', incident: { code: 'NRN-2048', type: 'PERSON_TRAPPED', severity: 'HIGH' }, status: 'IN_PROGRESS', district: DEFAULT_DISTRICT, createdAt: '2026-08-21T10:24:00.000Z' },
    { _id: 'citizen-report-2', incident: { code: 'NRN-2039', type: 'FLOODED_HOUSE', severity: 'MODERATE' }, status: 'VERIFIED', district: DEFAULT_DISTRICT, createdAt: '2026-08-21T08:42:00.000Z' },
    { _id: 'citizen-report-3', incident: { code: 'NRN-2017', type: 'MEDICAL_EMERGENCY', severity: 'HIGH' }, status: 'ASSIGNED', district: DEFAULT_DISTRICT, createdAt: '2026-08-20T19:18:00.000Z' }
];
function App() {
...
function AdminLogin({ onLogin, onBack }) {
    ...
    return <div className="admin-login-page">
        <header className="admin-login-topbar">
            <div className="brand"><IndianFlag /><div><div className="brand-name">RAHATSETU</div><div className="brand-sub">NATIONAL FLOOD RESPONSE PLATFORM</div></div></div>
            <button className="ghost admin-back" onClick={onBack}>Citizen Portal</button>
        </header>
...
function Alerts({ data }) { const { t } = useTranslation(); const [filter, setFilter] = useState('ALL'); const supplemental = [{ _id:'alert-more-1', title:'Heavy rainfall advisory', description:`Heavy rainfall may affect low-lying areas of ${data.district?.name || 'the district'}. Keep evacuation routes clear.`, severity:'WATCH' }, { _id:'alert-more-2', title:'River level monitoring alert', description:'River levels are being monitored closely. Review nearby shelters before travel.', severity:'WARNING' }, { _id:'alert-more-3', title:'Low-lying road access notice', description:'Waterlogging may affect local road access. Use marked response routes where available.', severity:'WATCH' }, { _id:'alert-more-4', title:'Shelter capacity watch', description:'Relief shelters are being monitored for available capacity and essential supplies.', severity:'WATCH' }, { _id:'alert-more-5', title:'Emergency preparedness notice', description:'Keep medicines, identification, drinking water and charged communication devices ready.', severity:'WATCH' }]; const allAlerts = data.alerts.length >= 6 ? data.alerts : [...data.alerts, ...supplemental.slice(0, 6 - data.alerts.length)]; const list = allAlerts.filter(a => filter === 'ALL' || a.severity === filter); return <><Header state={data.district?.state} district={data.district?.name} title={t('page.alerts.title')} sub={t('page.alerts.sub')} action={<div className="filters">{['ALL', 'ALERT', 'WARNING', 'WATCH'].map(x => <button className={filter === x ? 'selected' : ''} key={x} onClick={() => setFilter(x)}>{x}</button>)}</div>}/><div className="feed">{list.map(a => <Card key={a._id} className="feed-card"><div className="feed-icon warn"><AlertTriangle size={20}/></div><div><div className="feed-title"><b>{a.title}</b><span className={'severity ' + (a.severity === 'ALERT' ? 'critical' : a.severity === 'WARNING' ? 'high' : 'moderate')}>{a.severity}</span><Trust type={a.sourceType} name={a.source}/></div><p>{a.description}</p><small>Issued {a.issuedAt ? new Date(a.issuedAt).toLocaleString() : 'Recently'} • expires {a.expiresAt ? new Date(a.expiresAt).toLocaleString() : '—'}</small>{a.officialUrl && <a href={a.officialUrl} target="_blank" rel="noreferrer" className="official-link">Open official source ↗</a>}</div></Card>)}</div></>; }
function Weather({ district, state }) { const { t } = useTranslation(); const [w, setW] = useState(() => makeFallbackData(state, district).weather), [err, setErr] = useState(false); useEffect(() => { api(`/weather?state=${encodeURIComponent(state)}&district=${encodeURIComponent(district)}`).then(r => setW(r.data.data || makeFallbackData(state, district).weather)).catch(() => { setErr(true); setW(makeFallbackData(state, district).weather); }); }, [state, district]); return <><Header state={state} district={district} title={t('page.weather.title')} sub={t('page.weather.sub')}/><div className="stats"><Stat label="Temperature" value={w?.temperature != null ? w.temperature + '°' : '—'} icon={CloudRain}/><Stat label="Rainfall" value={w?.rainfallMm != null ? w.rainfallMm + ' mm' : '—'} icon={Activity} tone="amber"/><Stat label="Humidity" value={w?.humidity != null ? w.humidity + '%' : '—'} icon={CloudRain}/><Stat label="Warning" value={w?.warningLevel || '—'} icon={AlertTriangle} tone="red"/></div>{err && <div className="data-banner">Weather service unavailable — latest available weather information is shown.</div>}<Card className="forecast"><div className="card-head"><b>7-DAY DISTRICT OUTLOOK</b><Trust type={w?.sourceType} name={w?.source || 'Open-Meteo'}/></div><div className="forecast-grid">{w?.forecast?.map(f => <div key={f.day}><b>{f.day}</b><strong>{f.rainfallMm} mm</strong><span>{f.tempMin}° — {f.tempMax}°</span><small>{f.precipitationProbability}% precipitation probability</small></div>)}</div></Card></>; }
...
function Citizen({ user, state, setState, district, setDistrict, onLogout, onAdminLogin }) {
    ...
    const [data, setData] = useState(() => makeFallbackData(state, district)), [page, setPage] = useState('home'), [side, setSide] = useState(false), [reports, setReports] = useState(citizenSampleReports), [modal, setModal] = useState(false), [status, setStatus] = useState(null), [geo, setGeo] = useState(null), [loading, setLoading] = useState(true), [shelterInfo, setShelterInfo] = useState(null);
    ...
    const load = async () => {
        ...
        try {
            ...
            setReports([...(rr.data.data || []), ...citizenSampleReports]);
        }
        catch { }
        finally {
            setLoading(false);
    ...
    const navItems = [['home', 'nav.home', LayoutDashboard], ['map', 'nav.map', MapIcon], ['tracking', 'nav.tracking', Radio], ['shelters', 'nav.shelters', Warehouse], ['emergency', 'nav.emergency', Siren], ['evacuation', 'nav.evacuation', RouteIcon], ['weather', 'nav.weather', CloudRain], ['alerts', 'nav.alerts', ShieldAlert], ['reports', 'nav.myReports', CheckCircle2], ['safety', 'nav.safety', ShieldAlert]];
