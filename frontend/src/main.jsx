...
function Citizen({ user, state, setState, district, setDistrict, onLogout, onAdminLogin }) {
    ...
    const [data, setData] = useState(() => makeFallbackData(state, district)), [page, setPage] = useState('home'), [side, setSide] = useState(false), [reports, setReports] = useState([]), [modal, setModal] = useState(false), [status, setStatus] = useState(null), [geo, setGeo] = useState(null), [loading, setLoading] = useState(true);
 ...
 <div className="tracking-grid citizen-tracking">
  ...
  <Card><div className="card-head"><div><b>{t('citizen.tracking.rescueStatus')}</b><span>{t('citizen.tracking.rescueStatusSub')}</span></div><span className="live">{t('common.live')}</span></div>
   {data.resources.filter(r => r.status === 'EN_ROUTE' || r.status === 'BUSY').slice(0, 6).map(r => <div className="citizen-track-row" key={r._id}><div className="tracking-avatar"><Truck size={16}/></div><div><b>{r.name}</b><span>{r.type?.replaceAll('_', ' ')} • {r.status.replaceAll('_', ' ')}</span><small>{t('tracking.updatedContinuously')}</small></div><strong>{r.status === 'EN_ROUTE' ? t('tracking.enRouteLabel') : t('tracking.onScene')}</strong></div>)}
